<?php
session_start();
$db = mysqli_connect('localhost', 'root', '', 'blog_app');
...
function register()
{
    ...
    $dupli_result_username = mysqli_query($db, $check_duplicate_username);
    $dupli_count_username = mysqli_num_rows($dupli_result_username);

    $check_duplicate_email = "SELECT email FROM users WHERE email = '$email'";
    $dupli_result_email = mysqli_query($db, $check_duplicate_email);
    $dupli_count_email = mysqli_num_rows($dupli_result_email);

    // form validation: ensure that the form is correctly filled
    if (empty($username)) {
        array_push($errors, "-Username is required");
    } else if ($dupli_count_username > 0) {
        array_push($errors, "-Username Exist");
    }
    if (empty($email)) {
        array_push($errors, "-Email is required");
    } else if ($dupli_count_email > 0) {
    ...
    } else if (strlen($password_1) < 8 || !$number || !$uppercase || !$lowercase || !$specialChars) {
